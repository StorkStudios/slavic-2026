...
namespace CommandTerminal
{
    public static class BuiltinCommands
    {
        ...
        static void CommandHelp(CommandArg[] args) {
            if (args.Length == 0) {
                foreach (var command in Terminal.Shell.Commands) {
                    Terminal.AddLogEntry("{0}: {1}", command.Key.PadRight(16), command.Value.help);
                }
                return;
            }
            ...
            if (info.help == null) {
                Terminal.AddLogEntry("{0} does not provide any help documentation.", command_name);
            } else if (info.hint == null) {
                Terminal.AddLogEntry(info.help);
            } else {
                Terminal.AddLogEntry("{0}\nUsage: {1}", info.help, info.hint);
            }
        }
        ...
        static void CommandTime(CommandArg[] args) {
            ...
            sw.Stop();
            Terminal.AddLogEntry("Time: {0}ms", (double)sw.ElapsedTicks / 10000);
        }

        [RegisterCommand(Help = "Output message")]
        static void CommandPrint(CommandArg[] args) {
            Terminal.AddLogEntry(JoinArguments(args));
        }
        ...
        static void CommandTrace(CommandArg[] args) {
            ...
            if (log_count - 2 < 0) {
                Terminal.AddLogEntry("Nothing to trace.");
                return;
            }
            ...
            if (log_item.stack_trace == "") {
                Terminal.AddLogEntry("{0} (no trace)", log_item.message);
            } else {
                Terminal.AddLogEntry(log_item.stack_trace);
            }
        }
    #endif
        ...
        static void CommandSet(CommandArg[] args) {
            if (args.Length == 0) {
                foreach (var kv in Terminal.Shell.Variables) {
                    Terminal.AddLogEntry("{0}: {1}", kv.Key.PadRight(16), kv.Value);
                }
                return;
            }
            ...
            if (variable_name[0] == '$') {
                Terminal.AddLogEntry(TerminalLogType.Warning, "Warning: Variable name starts with '$', '${0}'.", variable_name);
            }
